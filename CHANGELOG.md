...
- Extract the ~30 errors Slack lists on every Web API method into a new `Common Errors` guide ([guides/common_errors.md](guides/common_errors.md)). Per-method docs now only list errors specific to that method and link to the shared guide, dramatically reducing noise in the generated docs. The doc-regeneration mix task derives the common-error set from the scraped pages, regenerates the guide and `lib/slack/web/common_errors.json`, and strips common errors from each per-method JSON.
